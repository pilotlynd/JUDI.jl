...
# and if applicable the input data amongst the available workers.
    ...
    # @sync begin
        for j=1:numSources
            ...
            elseif op=='F' && mode==-1
                results[j] = time_modeling(model, srcGeometryLocal, nothing, recGeometryLocal, copy(recData[j:j]), nothing, j, op, mode, opt_local)
            elseif op=='J' && mode==1
                results[j] = time_modeling(model, srcGeometryLocal, copy(srcData[j:j]), recGeometryLocal, nothing, perturbation, j, op, mode, opt_local)
            elseif op=='J' && mode==-1
                results[j] = time_modeling(model, srcGeometryLocal, copy(srcData[j:j]), recGeometryLocal, copy(recData[j:j]), nothing, j, op, mode, opt_local)
            end
        end
    # end
